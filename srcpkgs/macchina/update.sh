...
API_URL="https://api.github.com/repos/Macchina-CLI/macchina/releases/latest"
LATEST_TAG=$(curl -sL "$API_URL" | python3 -c "import sys,json; print(json.load(sys.stdin)['tag_name'])")

if [ "${CURRENT}" = "${LATEST_TAG#v}" ]; then
    echo "macchina: ${CURRENT} — already up to date"
    exit 0
fi

echo "macchina: ${CURRENT} → ${LATEST_TAG#v}"

DIST_URL="https://github.com/Macchina-CLI/macchina/releases/download/${LATEST_TAG}/macchina-${LATEST_TAG}-linux-gnu-x86_64.tar.gz"
echo "URL: ${DIST_URL}"
