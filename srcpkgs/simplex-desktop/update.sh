...
API_URL="https://api.github.com/repos/simplex-chat/simplex-chat/releases/latest"
LATEST_TAG=$(curl -sL "$API_URL" | python3 -c "import sys,json; print(json.load(sys.stdin)['tag_name'])")
LATEST_VER="${LATEST_TAG#v}"

if [ "${CURRENT}" = "${LATEST_VER}" ]; then
    echo "simplex-desktop: ${CURRENT} — already up to date"
    exit 0
fi

echo "simplex-desktop: ${CURRENT} → ${LATEST_VER}"

DEB_URL="https://github.com/simplex-chat/simplex-chat/releases/download/${LATEST_TAG}/simplex-desktop-ubuntu-22_04-x86_64.deb"
echo "URL: ${DEB_URL}"
