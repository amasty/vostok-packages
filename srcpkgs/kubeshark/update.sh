...
API_URL="https://api.github.com/repos/kubeshark/kubeshark/releases/latest"
LATEST_TAG=$(curl -sL "$API_URL" | python3 -c "import sys,json; print(json.load(sys.stdin)['tag_name'])")

if [ "${CURRENT}" = "${LATEST_TAG#v}" ]; then
    echo "kubeshark: ${CURRENT} — already up to date"
    exit 0
fi

echo "kubeshark: ${CURRENT} → ${LATEST_TAG#v}"

DIST_URL="https://github.com/kubeshark/kubeshark/releases/download/${LATEST_TAG}/kubeshark_linux_amd64"
echo "URL: ${DIST_URL}"

if ! curl --head --silent --fail "${DIST_URL}" > /dev/null; then
    echo "ERROR: Binary not found at ${DIST_URL}" >&2
    exit 1
fi
