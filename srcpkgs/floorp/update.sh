...
if [ "${CURRENT}" = "${LATEST_VER}" ]; then
    echo "floorp: ${CURRENT} — already up to date"
    exit 0
fi

echo "floorp: ${CURRENT} → ${LATEST_VER}"

DEB_URL="https://github.com/Floorp-Projects/Floorp/releases/download/${LATEST_TAG}/floorp-${LATEST_VER}.deb"
echo "URL: ${DEB_URL}"
