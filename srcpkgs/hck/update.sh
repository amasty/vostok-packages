...
API_URL="https://api.github.com/repos/sstadick/hck/releases/latest"
LATEST_TAG=$(curl -sL "$API_URL" | python3 -c "import sys,json; print(json.load(sys.stdin)['tag_name'])")

if [ "${CURRENT}" = "${LATEST_TAG#v}" ]; then
    echo "hck: ${CURRENT} — already up to date"
    exit 0
fi

echo "hck: ${CURRENT} → ${LATEST_TAG#v}"

DIST_URL="https://github.com/sstadick/hck/releases/download/${LATEST_TAG}/hck-linux-amd64"
echo "URL: ${DIST_URL}"
