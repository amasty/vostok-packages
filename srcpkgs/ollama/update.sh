...
if [ "${CURRENT}" = "${LATEST_VER}" ]; then
    echo "ollama: ${CURRENT} — already up to date"
    exit 0
fi

echo "ollama: ${CURRENT} → ${LATEST_VER}"

TAR_URL="https://github.com/ollama/ollama/releases/download/${LATEST_TAG}/ollama-linux-amd64.tar.zst"
echo "URL: ${TAR_URL}"
